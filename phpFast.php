...
class phpFastFile {
    ...
    function read($path, $relative_dir = true) {
        ...
        if (!$fail) {
            ...
            while (!feof($myfile)) {
                $index++;
                if ($index == 1) $string = fgets($myfile);
                else {
                    if (index == 2) $string = $string . '\n';
...
class phpFastString {
    ...
    function contains($str, $contains, $case_sensitive = false) {
        if ($case_sensitive) {
            ...
            if (strpos($str, $contains) !== false) return true;
            return false;
        }
        else {
            if (strpos(strtolower($str) , strtolower($contains)) !== false) return true;
            return false;
        }
    }
    /**
...
class phpFastDate {
    ...
    function today($year_first = False) {
        if ($year_first) return date('Y/m/d');
        return date("m/d/Y");
    }
    /**
...
class phpFastUser {
    ...
    private function check_pass($user, $pass) {
        if ($this->exists($user)) {
            $json = $this
                ->ffile
                ->json_read($this->get_active_path() . $user . '.json');
            foreach ($json as $attr => $val) {
    ...
    function register($user, $pass) {
        ...
        $pass_safe = password_hash($pass, PASSWORD_DEFAULT, ['cost' => $this->password_hash_cost]);
    ...
    function exists($user) {
        return $this
            ->ffile
            ->exists($this->get_active_path() . $user . '.json', $as_dir = True);
    }
    /**
    ...
    function delete($user) {
        if ($this->exists($user)) {
            $this
                ->ffile
                ->delete($this->get_active_path() . $user . '.json');
            $this
                ->ffile
                ->dir_r($this
                ->ffile
                ->path($this->get_active_path() . $user . '.json'));
            return true;
        }
        return false;
    }
    ...
    function edit($user, $key, $new_value, $force_edit = False) {
        ...
        if ($detected) {
            ...
            $this
                ->ffile
                ->json_write($this->get_active_path() . $user . '.json', $json);
            return true;
        }
